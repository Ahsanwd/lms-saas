function liveSessionScheduled({ recipientName, lessonTitle, courseName, scheduledAt, durationMinutes, platform, courseId, tenantName, branding, appUrl, tenantTimezone = 'UTC' }) {
  ...
  // Second, instructor-facing reference alongside the UTC anchor — the
  // tenant's own configured timezone (e.g. the school's home timezone),
  // shown only when it differs from UTC so the line isn't redundant.
