...
function resolveQuestions(quiz: QuizDetail, attempt: ActiveAttempt): QuizQuestion[] {
  ...
  return attempt.servedQuestions.map(q =>
...
function CreateQuestionModal({
  ...
  const [orderItems, setOrder]    = useState<string[]>(() =>
    initialData?.correctOrder?.length
      ? initialData.correctOrder.map(idx => initialData.options?.[idx]?.text ?? '')
      : ['', '', '']
  );
                    ...
                    <td className="px-4 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Button size="sm" variant="outline"
                          onClick={() => setViewingAttemptId(a._id)}>
                          View
                        </Button>
                        {a.status === 'pending_manual' && (
                          <Button size="sm" variant="outline"
                            onClick={() => setGradingAttempt(a)}>
                            Grade
                          </Button>
                        )}
                      </div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h1 className="text-xl font-semibold text-gray-900 truncate">{quiz.title}</h1>
