...
function StudentSettings() {
  ...
  // Which settings section is showing in the right panel
  const [activeTab, setActiveTab] = useState<'profile' | 'security' | 'sessions' | 'account' | '2fa'>('profile');
  ...
  const NAV_ITEMS: Array<{ key: typeof activeTab; label: string; desc: string; icon: React.ReactNode; badge?: number }> = [
    {
      key: 'profile', label: 'Profile', desc: 'Name & public info',
    ...
    {
      key: 'sessions', label: 'Sessions', desc: 'Active devices',
    ...
    {
      key: 'account', label: 'Account', desc: 'Read-only details',
    ...
    {
      key: '2fa', label: 'Two-Factor Auth', desc: authUser?.twoFactorEnabled ? 'Enabled' : 'Disabled',
      icon: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>,
    },
  ];

  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6 min-w-0 w-full">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start min-w-0">
        {/* ── Left nav ── */}
        <nav className="w-full md:w-60 min-w-0 flex-shrink-0 md:sticky md:top-6 flex flex-row md:flex-col gap-1 overflow-x-auto md:overflow-visible pb-1 md:pb-0">
          {NAV_ITEMS.map(item => (
            <button
              key={item.key}
              type="button"
              ...
              className={cn(
                'flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors flex-shrink-0 md:w-full',
                activeTab === item.key ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-50'
              )}
            >
              <span className={cn('w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0',
                activeTab === item.key ? 'bg-primary-100 text-primary-600' : 'bg-gray-100 text-gray-400')}>
                {item.icon}
              </span>
              ...
              <span className="text-sm font-semibold md:hidden whitespace-nowrap">{item.label}</span>
              {item.badge !== undefined && item.badge > 0 && (
                <span className="text-[10px] font-bold bg-gray-100 text-gray-500 rounded-full px-1.5 py-0.5 flex-shrink-0">{item.badge}</span>
              )}
            </button>
          ))}
        </nav>
