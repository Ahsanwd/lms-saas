...
export default function TenantAdminDashboardPage() {
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-gray-50">
              {stats.popularCourses.map((course: { _id: string; title: string; enrollmentCount: number; rating?: { average: number } }) => (
                <div key={course._id} className="px-6 py-4 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-900">{course.title}</p>
                    <p className="text-xs text-gray-400 mt-0.5">{course.enrollmentCount} students enrolled</p>
                  </div>
                  <Badge variant="success">★ {course.rating?.average?.toFixed(1) ?? 'N/A'}</Badge>
                </div>
              ))}
            </div>
          </CardContent>
      ...
      {/* Quiz Stats */}
      {(stats?.quizzes?.totalAttempts ?? 0) > 0 && (
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                <p className="text-2xl font-bold text-gray-900">{stats.quizzes.totalAttempts}</p>
              ...
              <div>
                <p className="text-2xl font-bold text-gray-900">{stats.quizzes.passRate}%</p>
